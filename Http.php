...
class Http {
	 ...
	 * @return string
	 */
	static public function curlPost($url, $post_data=array(), $timeout=50,$header="") {
	    error_reporting(E_ALL);
	    ini_set('display_errors', 'on');

		$header=self::defaultHeader();
		$post_string = is_string($post_data) ? $post_data : http_build_query($post_data);
        $header=self::defaultHeader();
		//var_dump($url, $post_data, );;
		$ch = curl_init();
		...
		$result = curl_exec($ch);
        file_put_contents("./log.log", $url .'->'. json_encode($post_data) .'->'. $result . curl_error($ch). "\n" , 8);
        curl_close($ch);
	...
	static public function phpPost($url, $post_data=array(), $timeout=5,$header="") {
		...
		var_dump($url, $opts, $header, $post_data, file_get_contents($url,false,$context));exit;
		return  file_get_contents($url,false,$context);
	}
	...
	static private function defaultHeader() {
		$header="User-Agent:Dalvik/2.1.0 (Linux; U; Android 6.0.1; Redmi Note 3 MIUI/8.11.22)\r\n";
    ...
    public static function postJson($url, $data) {
        ...
        try {
            ...
            Log::info("POST-JSON:$url " . json_encode($data));
            $content =  file_get_contents($url, false, $cxContext);
            Log::info("POST-JSON:$url 返回 " . $content);
            return $content;
        }catch (Exception $e) {
            Log::info("POST-JSON:$url 返回 " . $e->getMessage());
            return false;
        }
